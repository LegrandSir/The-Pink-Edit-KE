      ...
      {isCartOpen && (
        <>
          ...
          <motion.div 
            ...
            // THE FIX IS HERE: w-full for mobile, w-[400px] for md screens
          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              ...
              <X onClick={toggleCart} className="w-6 h-6 cursor-pointer hover:text-pink-500 transition-colors" />
            </div>
              ...
              {cartItems.map(item => (
                <div key={item.id} className="flex gap-4">
                  ...
                  <div className="flex flex-col justify-between flex-1 py-1">
                    <div>
                      <h3 className="font-bold text-sm text-gray-900">{item.name || item.title}</h3>
                    ...
                    <div className="flex justify-between items-end">
                      ...
                      <span className="font-medium text-sm">Ksh {(item.price * item.qty).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="border-t border-gray-200 p-6 bg-[#faf8f8]">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="font-serif text-2xl">Ksh {cartTotal.toLocaleString()}</span>
              </div>
              <p className="text-xs text-gray-400 mb-6 text-center italic">Shipping calculated at checkout.</p>
              
              <button 
                onClick={handleCheckoutClick}
                className="w-full bg-pink-500 text-white font-bold uppercase tracking-widest py-4 rounded-sm hover:bg-pink-600 transition-colors flex items-center justify-center gap-2 text-sm"
              >
